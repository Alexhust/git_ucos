...
void  OSTimeTickHook (void)
{
    ...
    if (OS_AppTimeTickHookPtr != (OS_APP_HOOK_VOID)0) {
        (*OS_AppTimeTickHookPtr)();         //ִ??OS_AppTimeTickHookPtrָ??ָ???ĺ???
                                            //?ں???App_OS_SetAllHooks??????ָ??
    }
#endif
}
...
void  OS_CPU_SysTickInit (CPU_INT32U  cnts)
{
    ...
    prio &= DEF_BIT_FIELD(24, 0);                           /*DEF_BIT_FIELD(x,y) sets the BIT between x and y         */ 
    prio |= DEF_BIT_MASK(OS_CPU_CFG_SYSTICK_PRIO, 24);

    CPU_REG_NVIC_SHPRI3 = prio;                             /*set the Priority of sysTick exception 0                 */
